...
import { Calendar, Clock, MapPin, Users, Tag, Globe, ArrowLeft, Share2, Heart, ChevronDown, ChevronUp } from 'lucide-react';
...
const EventDetailPage = () => {
  ...
  useEffect(() => {
    ...
    if (event) {
      document.title = `${event.title} - TechEvents`;
    } else {
      document.title = "Event Not Found - TechEvents";
    }
  }, [event]);
                    ...
                    <div className="prose prose-lg max-w-none text-gray-700 mb-6">
                      ...
                      {event.description.length > 200 && (
                        <button 
                          ...
                          className="flex items-center text-primary-600 font-medium hover:text-primary-700 transition-colors mt-2"
                        >
                          {showFullDescription ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                      {event.judges.map((judge, index) => (
                        <div key={index} className="flex items-center space-x-4">
                          ...
                          />
                          <div>
                            <h4 className="font-bold text-gray-900">{judge.name}</h4>
                            <p className="text-gray-600">{judge.role}</p>
                            <p className="text-gray-500 text-sm">{judge.company}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8 sticky top-24">
              <div className="p-6">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="flex items-start">
                    <Users className="h-5 w-5 mr-3 text-primary-500 flex-shrink-0 mt-0.5" />
                    <div>
                ...
                <div className="border-t border-gray-200 pt-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Ticket Prices</h3>
                  <div className="space-y-3">
                    {event.prices.map((price, index) => (
                      <div key={index} className="flex justify-between items-center">
                        <span className="text-gray-600">{price.split('-')[0].trim()}</span>
                        <span className="font-medium text-gray-900">{price.split('-')[1].trim()}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* {event.earlyBirdDeadline && (
                    <p className="text-sm text-gray-500 mt-4">
                      Early bird pricing ends on {new Date(event.earlyBirdDeadline).toLocaleDateString()}
                    </p>
                  )} */}
                </div>
                ...
                <div className="border-t border-gray-200 pt-6 mt-6" id="register">
                  ...
                  <p className="text-center text-sm text-gray-500 mt-4">
                    Already have an account? <Link to="/login?role=participant" className="text-primary-600 hover:text-primary-700 transition-colors">Sign in</Link>
                  </p>
                </div>
              </div>
            </div>
